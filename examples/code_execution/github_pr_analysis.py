...
many LLM roundtrips while code execution handles in a single loop.
    ...
    Level 2:  Per PR, fetch reviews + files       -> 60 API calls (30 x 2)
                                                    ~= 62 total

Traditional parallel tool calling (best case):
    Roundtrip 1:  list PRs                        -> 30 PR objects enter context
    Roundtrip 2:  30x get_pr_reviews in parallel  -> 30 review payloads enter context
    Roundtrip 3:  30x get_pr_files in parallel    -> 30 file-list payloads enter context
...
Run:
    uv run -m examples.code_execution.github_pr_analysis
"""
