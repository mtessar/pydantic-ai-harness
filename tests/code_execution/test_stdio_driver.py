...
from pydantic_harness.toolsets.code_execution._driver import (
...
DRIVER_PATH = (
    Path(__file__).parents[2] / 'pydantic_harness' / 'toolsets' / 'code_execution' / '_driver.py'
)
...
def test_compile_code_syntax_error(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr('pydantic_harness.toolsets.code_execution._driver._write_msg', messages.append)
