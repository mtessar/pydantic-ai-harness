...
from pydantic_ai.messages import tool_return_ta  # private API
from pydantic_ai import exceptions
from pydantic_ai._run_context import AgentDepsT, RunContext  # private API
from pydantic_ai._tool_manager import ToolManager, _parallel_execution_mode_ctx_var  # pyright: ignore[reportPrivateUsage]  # private API
from pydantic_ai.exceptions import ApprovalRequired, CallDeferred, ModelRetry
from pydantic_ai.messages import ToolCallPart
from pydantic_ai.tools import ToolDefinition
from pydantic_ai.toolsets.abstract import AbstractToolset, SchemaValidatorProt, ToolsetTool
...
def get_environment(name: EnvironmentName) -> ExecutionEnvironment:
    ...
    Args:
        name: The environment name (`'monty'`).
...
class CodeExecutionToolset(AbstractToolset[AgentDepsT]):
    ...
    Args:
        environment: The code execution environment. Can be an environment instance or a string
            shorthand (`'monty'`). Defaults to `'monty'`.
    ...
    async def get_tools(self, ctx: RunContext[AgentDepsT]) -> dict[str, ToolsetTool[AgentDepsT]]:
        ...
        if wrapped_tools:
            ...
            for sanitized_name, original_name in name_map.items():
                tool_def = wrapped_tools[original_name].tool_def
